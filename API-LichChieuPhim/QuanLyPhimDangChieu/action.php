...
<body>
  ...
  {
    try {
      $stmt = $conn->prepare("DELETE FROM DanhSachPhim WHERE id='".$_GET['xoa']."'");
      $stmt->execute();
      echo "Xoá thành công";
  ...
  if(isset($_POST['id']))
  {
    ...
    {
      try {
        $stmt = $conn->prepare("INSERT INTO DanhSachPhim (TenPhim,TheLoai,DaoDien,DienVien,NhaSanXuat,ThoiLuong,KhoiChieu,NoiDung,HinhAnh,Trailer,Galaxy,CGV,Lotte,Rate) VALUES (:TenPhim,:TheLoai,:DaoDien,:DienVien,:NhaSanXuat,:ThoiLuong,:KhoiChieu,:NoiDung,:HinhAnh,:Trailer,:Galaxy,:CGV,:Lotte,0");
      ...
      try {
        $stmt = $conn->prepare("UPDATE DanhSachPhim SET TenPhim=:TenPhim,TheLoai=:TheLoai,DaoDien=:DaoDien,DienVien=:DienVien,NhaSanXuat=:NhaSanXuat,ThoiLuong=:ThoiLuong,KhoiChieu=:KhoiChieu,NoiDung=:NoiDung,HinhAnh=:HinhAnh,Trailer=:Trailer,Galaxy=:Galaxy,CGV=:CGV,Lotte=:Lotte WHERE id='".$_POST["id"]."'");
        $stmt->bindParam(':TenPhim', $_POST['TenPhim']);
        $stmt->bindParam(':TheLoai', $_POST['TheLoai']);
        $stmt->bindParam(':DaoDien', $_POST['DaoDien']);
        $stmt->bindParam(':DienVien', $_POST['DienVien']);
  ...
  if(isset($_GET['sua']))
  {
    if ($_GET['sua']=="new")
    {
      ...
      <?php
              ...
              try {
                $stmt = $conn->prepare("SELECT * FROM DanhSachPhim WHERE id='".$_GET["sua"]."'");
                $stmt->execute();
