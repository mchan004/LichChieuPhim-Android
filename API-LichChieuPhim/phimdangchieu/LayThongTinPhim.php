...
try {
  ...
  $stmt_Add = $conn->prepare("UPDATE DanhSachPhim SET KhoiChieu=STR_TO_DATE(:KhoiChieu, '%d.%m.%Y'), TheLoai=:TheLoai, NoiDung=:NoiDung, DienVien=:DienVien, DaoDien=:DaoDien, NhaSanXuat=:NhaSanXuat, ThoiLuong=:ThoiLuong, HinhAnh=:HinhAnh, Trailer=:Trailer WHERE id=:id");
  ...
  $stmt = $conn->prepare("SELECT id,Galaxy FROM DanhSachPhim WHERE Galaxy<>'null'");
  $stmt->execute();
    ...
    foreach($result as $row) {
      ...
      foreach($elements as $e)  {
        ...
        $k = trim(html_entity_decode($e->find('strong')[0]->plaintext));
          switch ($k) {
            case "Khởi chiếu:":
                $KC = mb_substr($e->plaintext, 15, 10);
                break;
            case "Thể loại:":
      ...
      $ND = EditText($html->find('.desc section p')[0]->plaintext);
      $id = $row['id'];
      $stmt_Add->execute();
    }
///////////////////////
...
//////CGV//////////////
///////////////////////
    $stmt = $conn->prepare("SELECT id,CGV FROM DanhSachPhim WHERE Galaxy='null' AND CGV<>'null'");
    $stmt->execute();
    $result = $stmt->fetchAll();
    foreach($result as $row) {
      ...
      $id = $row['id'];
      $TL = EditText($html->find('.product-essential .movie-genre .std')[0]->plaintext);
      $DV = EditText($html->find('.product-essential .movie-actress .std')[0]->plaintext);
      $DD = EditText($html->find('.product-essential .movie-director .std')[0]->plaintext);
      $TLP= EditText($html->find('.product-essential .movie-actress .std')[1]->plaintext);
      $KC = EditText($html->find('.product-essential .movie-release .std')[0]->plaintext);
      $KC = str_replace("/",".",$KC);
      $NSX = "null";
      $ND = addslashes(EditText($html->find('.product-collateral .std')[0]->plaintext));
